...
class _MyDialog extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    _dialog = _MyDialogState(id: id, dialog: dialog, packetList: packetList);
  ...
  Widget build(BuildContext context) {
    if (_dialogType == Dialogs.YES_OPTION) {
      ...
      return AlertDialog(
        ...
        actions: <Widget>[
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: PitikColors.primaryOrange),
            onPressed: () {
              dialog.hide();
              _listener!.onDialogOk(context, id, packetList);
            },
            child: Text(_titleButtonOk, style: const TextStyle(color: Colors.white)),
          ),
          ElevatedButton(
      ...
      return AlertDialog(shape: const RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(8))), title: Text(_title), content: Text(_message));
    }
  }
}
